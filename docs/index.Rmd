...
df_samples_treatments$treatment <- ""
for (row in (1:nrow(df_samples_treatments))){
...
for(row_scores in 1:nrow(pca_scores)){
  for(row_df in 1:nrow(df_samples_treatments)){
    if(pca_scores[row_scores, ncol(pca_scores)-1] == df_samples_treatments[row_df, 2]){
      pca_scores[row_scores, ncol(pca_scores)] <- df_samples_treatments[row_df, 3]
    }
  }
}

ggplot(pca_scores, aes(x=PC1, y=PC2, colour = inflammation)) +
  geom_point()
```
...
```{r fig.width=14}
ggplot(pca_scores_lognorm, aes(x=PC1, y=PC2, colour = inflammation)) +
  geom_point(size=2)
```
...
top10_pos_pca_lognorm <- pca_lognorm_rotations %>% arrange(desc(PC1)) %>% select(c(PC1, gene_name))
top10_pos_pca_lognorm <- top10_pos_pca_lognorm[1:10,]
top10_pos_pca_lognorm$gene_name
```

```{r}
# Show genes with more negative rotation values for pca (raw counts)
top10_neg_pca_lognorm <- pca_lognorm_rotations %>% arrange(PC1) %>% select(c(PC1, gene_name))
top10_neg_pca_lognorm <- top10_neg_pca_lognorm[1:10,]
top10_neg_pca_lognorm$gene_name
```

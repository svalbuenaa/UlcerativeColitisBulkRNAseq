...
plot_CoV_mean <- function(arg_dataframe, arg_x, arg_y,
                          ...
                          arg_color = "blue"){
  ggplot(arg_dataframe, aes_string(x = arg_x, y = arg_y, label="Gene")) +
...
p1 <- suppressMessages(ggplot(gene_list_sig_down_outlier, aes(x=log2FoldChange, y=-log10(padj), label=Gene)) + 
  geom_point(size=0.8, color="#6696ea") + 
  xlim(-31, -19) + 
...
p2 <- suppressMessages(ggplot(gene_list%>%filter(log2FoldChange>(-20)), aes(x=log2FoldChange, y=-log10(padj), label=Gene, color=significance))+ 
  geom_point(size=0.8) + 
  xlim(-12, 10)+ 
...
p1plotly <- ggplotly(p1) %>% layout(height = 480)

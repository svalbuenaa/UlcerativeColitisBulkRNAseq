...
location_matrix <- metadata_matrix_2[,c(7,11)]
location_matrix$`!Sample_source_name_ch1` <- str_replace_all(location_matrix$`!Sample_source_name_ch1`, "Colon_", "")
location_matrix$`!Sample_characteristics_ch1` <- str_replace_all(location_matrix$`!Sample_characteristics_ch1`, "location: ", "")
rownames(location_matrix) <- location_matrix$`!Sample_source_name_ch1`
colnames(location_matrix) <- c("Sample", "Location")
t_genes_list_norm$Location <- as.factor(location_matrix[rownames(t_genes_list_norm),"Location"])
```
...
plotSingleGenes <- function(arg_gene){
  ...
  ggplot(t_genes_list_norm, aes(y=get(arg_gene), x = Location, fill=Inflammation))+
